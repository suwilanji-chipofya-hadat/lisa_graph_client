...
export default function Market() {
  ...
  const [topGainersEmpty, setTopGainersEmpty] = useState(false);
  ...
  if (tg_error) {
    return <p>{tg_error.message}</p>;
  }

  if (i_error) {
    return <p>{i_error.message}</p>;
  }
  ...
  useEffect(() => {
    ...
    if (top_losers_data) {
      if (!top_losers_data.topLosers.length) {
        setTopLosersEmpty(true);
      }
    }
    if (top_gainers_data) {
      if (!top_gainers_data.topGainers.length) {
    ...
    <div className="w-full h-full overflow-y-scroll pb-[200px]">
      <div className="p-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 place-items-center gap-3">
          {top_gainers_data &&
            top_gainers_data.topGainers.map((stock) => (
              <div
                key={stock.instrument}
                className="flex w-full bg-gray-800 p-3 rounded-md justify-between"
              >
      ...
      {!topLosersEmpty && (
        <div className="p-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 place-items-center gap-3">
            {top_losers_data &&
              top_losers_data.topLosers.map((stock) => (
                <div
                  key={stock.instrument}
                  className="flex space-x-8 w-full sm:w-[200px] bg-gray-800 p-3 rounded-md"
                >
      ...
      <div>
        ...
        <div className="place-items-center gap-3 grid md:grid-cols-2 lg:grid-cols-4 p-3">
          {stock_data &&
            stock_data.stockData.map((stock, i) => (
              <StockCard key={i} stock={stock}></StockCard>
            ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-3 place-items-center">
          {instrument_data &&
            instrument_data.instruments.map((ins, i) => (
              <div
                key={i}
                className="p-4 bg-gray-800 w-full h-full rounded-md bg-gradient-to-br from-gray-300 via-gray-700 to-gray-900"
              >
